...
fact that is not derived from the chronicle, which is what makes the compiled
...
HEADER = (
    "<!-- Generated by `prokron compile`. Do not edit. "
    f"Edit `{AUTHORITY_DIR}/` instead. -->\n"
)
...
`{AUTHORITY_DIR}/`. Nothing here is authoritative and nothing here should be
edited by hand. Delete the directory and run `prokron compile` to rebuild it.
...
def state(project: Project, report: Report) -> str:
    ...
    lines += [
        ...
        f"`{AUTHORITY_DIR}/HANDOFF.md`. This file reports only what compiles.",
        "",
    ]
    return "\n".join(lines)
