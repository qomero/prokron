# /prokron-resume

Recover the project from the chronicle alone. Start with
`./bin/prokron status`, then read `.prokron/STATE.md`,
`prokron/PHASES.md`, `.prokron/TASK_GRAPH.md`, the active or selected task, its
contract in `ACCEPTANCE.md`, its governing ADRs, `INTENT.md`, `HANDOFF.md`, and
only the recent journal entries needed for continuity. State the current goal and exact next
action, then continue. Read implementation code only after the chronicle points
to the work that requires it. Treat the chronicle as current project truth and
the product specification as intended behavior that may require reconciliation.
