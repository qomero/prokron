...
## AC-T-UPGRADE-01 — Refresh generated views when the tool is upgraded

Reported from real use: after upgrading, the dashboard was the one the previous
release had written, so a shipped feature looked missing. No fixture had ever
installed over existing generated output.

- `AC-T-UPGRADE-01-01` — Given an installation whose generated views were
  written by an older release, When the installer runs, Then the views are
  regenerated by the new runtime. `TEST` · `PASS`
  - Evidence: reproduced against the real distribution before anything was
    changed — a v0.3.0 installation upgraded through the published one-liner
    kept its v0.3.0 dashboard, with none of the interactive code the release
    notes describe. With the fix, the same upgrade reports `Generated views
    were refreshed by the new runtime` and the page carries it. The installer
    suite now replaces two generated files with a marker, upgrades, and asserts
    the marker is gone and that `project.json` records the version that just
    installed. Mutation-verified: disabling the refresh fails the suite.
- `AC-T-UPGRADE-01-02` — Given a fresh installation, When the installer runs,
  Then no compiled directory is created; compiled state still appears only
  after compiling. `TEST` · `PASS`
  - Evidence: the refresh runs only when the installation already has a
    compiled directory and the chronicle holds tasks. The existing assertion
    that a repaired partial installation has no compiled directory still
    passes, and the guard was demonstrated by a first attempt at this test
    whose fixture had no tasks: the refresh correctly declined to run.
- `AC-T-UPGRADE-01-03` — Given an installation whose authority does not
  validate, When the installer runs, Then installation still succeeds and the
  refresh failure is reported with the command to run. `TEST` · `PASS`
  - Evidence: the suite appends a task with an unknown dependency, upgrades,
    and asserts the installer exits zero, reports `could not be refreshed`,
    names `prokron compile`, and leaves the broken task in place. Records are
    never at risk here, because the only files the refresh writes are ones the
    chronicle reproduces.
- `AC-T-UPGRADE-01-04` — Given compiled output, When it is read, Then it
  records which version of the tool produced it. `TEST` · `PASS`
  - Evidence: `project.json` carries `generatorVersion`, read from the version
    beside the runtime rather than from the tracked project. The suite asserts
    a refreshed installation records the version that just installed.
- `AC-T-UPGRADE-01-05` — Given compiled output written by a different version,
  When `status` runs, Then it reports the mismatch and names the command that
  fixes it. `TEST` · `PASS`
  - Evidence: the suite rewrites `generatorVersion` to `0.0.1` and asserts
